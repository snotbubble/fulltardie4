...
// - [ ] check screen size/dpi, move params to a popup if screen is above 150% scaling
...
// padding strings, appending the owner int to post-process after
	...
	for (var u = 0; u < fdat.length; u++) {
		...
		txt = ((string) ch) + " : " + sct + " : " + amt + " : " + sds + ";" + (("%d").printf(rfd.frm));
		forecasted += txt;
...
// this is a stupid workaround to vala's basic array handling
	...
	for (var r = 0; r < forecasted.length; r++) {
		if (forecasted[r] != null || forecasted[r].length > 0) {
			...
			lbl.set_tooltip_text(forecasted[r].split(";")[1]);
			var mqq = "".concat("<span font='monospace 12px'><b>", forecasted[r].split(";")[0].concat(" : ", ("%.2lf").printf(rut)), "</b></span>");
			lbl.set_markup(mqq);
			w.add(lbl);
		}
	}
...
public class FTW : Window {
	...
	string[,] dat = {
		...
		{"14","14","10","0","1","0","0","-15.0","utility","home","every 14th and 28th day of every month or the following weekday if on a weekend"}
	};
...
// setup page
		...
		for (var e = 0; e < dat.length[0]; e++) {
			...
			ll.xalign = ((float) 0.0);
// color test
// remove and apply to group list instead
			Random.set_seed(e);
			float[] hh = { ((((float) e) / ((float) dat.length[0])) * ((float) 255.0)), ((float) 0.8), ((float) 0.5) };
			int[] clr = hsvtorgb(hh);
// apply markup to label
			var mqq = "".concat("<span color='", htmlcol(clr[0], clr[1], clr[2]), "' font='monospace 16px'><b>", dat[e,10], "</b></span>");
...
// selecting row by index doesn't work as expected, have to mine & decipher crazy OOP github code learn out how to do this...

		//setuplist.select_row(0);
					...
					if (exts[1] == "scenario") {
						...
						muh.clicked.connect ((buh) => {
							...
							if (tt != null) {
								...
								string[,] tdat = new string[oo.length,11];
