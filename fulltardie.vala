...
void adjustgroupcolor (string[,] d, string[,] f, Gtk.Button t, int e, Entry h, double r, double g, double b, bool x, int ind) {
	var tabi = ("%-" + ind.to_string() + "s").printf("");
...
// also update forecasted, matching group only
		...
		t.override_background_color(NORMAL, c);
	}
}

// select a row, update params
...
// graph interaction
		...
		ges.set_propagation_phase(Gtk.PropagationPhase.CAPTURE);
		ges.set_button(0);
		ges.pressed.connect(() => {
			print("ges.pressed.connect...\n");
		});
		ges.released.connect(() => {
			print("ges.released.connect...\n");
		});
...
// non-rule changing edits - just update dat & re-render the forecast list & graph

		grpcombo.changed.connect(() => {
			if (doupdate) {
				ind = 4;
				var n = grpcombo.get_active_text().strip();
				var s = setuplist.get_selected_row();
				var r = 0;
				if (s != null) {
...
// grab group color from another item with the same group, if one exists
					...
					for  (int i = 0; i < dat.length[0]; i++) {
						...
						if (i != r) { 
							if (dat[i,9] == n) {
								uc = dat[i,12];
								//print("grpcombo.changed.connect:\tdat[%d,9] = %s\n", i, dat[i,12]);
					...
					if (uc == "") { uc = textcolor(); }
					gc.parse(uc);
					grpcolb.override_background_color(NORMAL, gc);

// save group color, re-render the setup list

					dat[r,12] = uc;
					if (notebook.get_current_page() == 0) { rendersetuplist(dat, setuplist, ind); }

// inject group and groupcolor into forecasted
...
// alo redraw the graph if we're looking at it
...
// alo redraw the graph if we're looking at it
					...
					if (dsc.text.strip() != "") {
						...
						if (s != null) {
							...
							if (notebook.get_current_page() == 1) {
								var fs = forecastlistbox.get_selected_row();
								var fr = 0;
								if (fs != null) { fr = fs.get_index(); }
								forecasted[fr,1] = dsc.text.strip();
								renderforecast(forecasted, forecastlistbox, ind);
							}
							doupdate = true;
						}
		...
		grpcolb.button_press_event.connect(() =>  {
			...
			if (s != null) {
				...
				r = s.get_index();
				string h = dat[r,12];
				if (h.strip() == "") { h = textcolor(); print("    grpcolb.button_press_event.connect:\tgroup color data not found: %s", dat[r,12]); }
				var g = new Gdk.RGBA();
				if (g.parse(h)) {
					doupdate = false;
					hhh.text = h;
		...
		rrr.adjustment.value_changed.connect(() => {
			if (doupdate) {
				...
				if (s != null) {
					...
					r = s.get_index();
					adjustgroupcolor(dat, forecasted, grpcolb, r, hhh, rrr.adjustment.value, ggg.adjustment.value, bbb.adjustment.value, false, ind);
